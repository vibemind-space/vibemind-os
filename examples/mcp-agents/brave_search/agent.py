...
# ========== Defaults ==========
DEFAULT_SYSTEM_PROMPT = """You are an AutoGen Assistant with access to brave_search MCP tools.
Use the available tools to complete brave_search tasks.
Follow the TOOL USAGE contract strictly and call only the exposed tool names.
"""

DEFAULT_TASK_PROMPT = """Use the available brave_search tools to accomplish the goal and stream your progress.
Be clear and concise in your responses.
"""

DEFAULT_OPERATOR_PROMPT = """ROLE: Brave Search Operator
GOAL: Complete brave_search tasks using available MCP tools.
TOOLS: Use ONLY the available MCP brave_search tools.
GUIDELINES:
...
GOAL: Verify that the user's brave_search task is completely and correctly fulfilled.
CHECK:
- Were the required brave_search operations precisely executed?
...
def get_system_prompt() -> str:
    """Get system prompt for brave_search agent."""
...
def get_task_prompt() -> str:
    """Get task prompt for brave_search operations."""
...
def init_model_client(task: str = "") -> OpenAIChatCompletionClient:
    ...
    return shared_init_model_client("brave_search", task)


# ========== Pydantic Config Model ==========
class BraveSearchAgentConfig(BaseModel):
    """Configuration for brave_search agent execution."""
    task: str
    session_id: str
    name: str = "brave-search-session"
    model: Optional[str] = None
    keepalive: bool = False


# ========== Main Entry Point (SESSION_ANNOUNCE Pattern) ==========
async def run_brave_search_agent(config: BraveSearchAgentConfig):
    """Brave Search MCP Agent main entry point.

    Follows the SESSION_ANNOUNCE pattern for backend integration.

    Args:
        config: BraveSearchAgentConfig with task, session_id, etc.
    """
    # Setup logging with session identifier
    logger = setup_logging(f"brave_search_agent_{config.session_id}")

    # Initialize EventServer with session logging
    event_server = EventServer(session_id=config.session_id, tool_name="brave_search")
    ...
    tool_config = None
    for srv in servers:
        if srv.get("name") == "brave-search" and srv.get("active"):
            tool_config = srv
            break

    if not tool_config:
        event_server.broadcast("error", {"text": "Brave Search MCP server not found or not active in servers.json"})
    ...
    tool_secrets = secrets.get("brave-search", {})
    ...
    async with McpWorkbench(server_params) as mcp:
        ...
        operator_prompt = load_prompt_from_module("brave_search_operator_prompt", BASE_DIR, DEFAULT_OPERATOR_PROMPT)
        ...
        event_server.broadcast("status", {"text": "Society of Mind: Brave Search Operator + QA Validator"})
        ...
        print(f"\n{'='*60}")
        print(f"🎭 Society of Mind: Brave Search Operator + QA Validator")
        print(f"{'='*60}\n")
        ...
        try:
            ...
            async for message in team.run_stream(task=full_prompt):
                ...
                if hasattr(message, 'source') and hasattr(message, 'content'):
                    ...
                    if source == "BraveSearchOperator":
                        print(f"\n🔧 BraveSearchOperator:")
                        print(f"   {content[:500]}{'...' if len(content) > 500 else ''}")
                        if event_server:
                            event_server.broadcast("agent.message", {
                                "agent": "BraveSearchOperator",
                                "role": "operator",
                                "content": content,
                                "icon": "🔧"
                            })
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Brave Search MCP Agent with Society of Mind")
    ...
    parser.add_argument("--name", default="brave-search-session", help="Agent session name")
    ...
    # Determine task
    task = args.task or os.getenv("MCP_TASK") or "Complete brave_search task"

    # Create config
    config = BraveSearchAgentConfig(
        task=task,
    ...
    asyncio.run(run_brave_search_agent(config))
