...
try:
    import dotenv
    # Path: mcp_plugins/servers/taskmanager/agent.py -> go up 3 levels to project root
...
class TaskManagerMCPAgent:
    ...
    def __init__(self, config: TaskManagerAgentConfig):
        ...
        self.server_params = None  # MCP server params for McpWorkbench
    ...
    async def initialize(self):
        """Initialize the agent with event server and MCP server params"""
        try:
            # Start event server
            ...
            # Initialize model client with intelligent routing
            self.model_client = init_model_client("taskmanager", self.config.task)
            self.logger.info(f"Model client initialized for taskmanager")
    ...
    def _create_team(self, task_mcp_workbench):
        """Create Society of Mind team with TaskManager-specific agents"""
        # Note: tools cannot be used with workbench in autogen
        # ask_user_tool = create_ask_user_tool(self.event_server, self.session_id)

        # Task Operator Agent - Main task management expert (with MCP workbench)
        task_operator = AssistantAgent(
            name="Task_Operator",
            model_client=self.model_client,
            workbench=task_mcp_workbench,
    ...
    async def run_task(self):
        """Run the TaskManager task using McpWorkbench"""
        try:
            await self.event_server.send_event({
            ...
            # Use McpWorkbench context for MCP operations
            async with McpWorkbench(self.server_params) as task_mcp:
                # Create team with MCP workbench
                self._create_team(task_mcp)
    ...
    async def cleanup(self):
        """Clean up resources"""
        try:
            # McpWorkbench auto-cleans via context manager
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="TaskManager MCP Agent with Society of Mind")
    parser.add_argument("--task", default="List all tasks", help="Task for the agent to execute")
    parser.add_argument("--session-id", dest="session_id", default=None, help="Session identifier")
    parser.add_argument("--name", default="taskmanager-session", help="Agent session name")
    ...
    parser.add_argument("--task-file", dest="task_file_path", default=None, help="Path to tasks.json file")
    ...
    try:
        ...
        if args.config_json:
            ...
            config = TaskManagerAgentConfig(
                session_id=args.session_id or f"taskmanager_{uuid.uuid4().hex[:8]}",
                name=args.name,
                model=args.model,
                task=args.task,
