#!/usr/bin/env python3
"""Simulate Prisma MCP Agent with a realistic task."""
import subprocess
import sys
import os

def main():
    """Run prisma agent simulation."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(script_dir)))

    print("=" * 60)
    print("Prisma Agent Simulation")
    print("=" * 60)
    print("Task: Read and describe Prisma schema")
    print()

    try:
        result = subprocess.run(
            [sys.executable, os.path.join(script_dir, "agent.py"),
             "--task", "Read and describe the Prisma schema structure",
             "--session-id", "sim_prisma_test",
