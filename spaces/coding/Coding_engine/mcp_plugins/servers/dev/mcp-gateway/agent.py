...
DEFAULT_GITHUB_OPERATOR_PROMPT = """ROLE: MCP Gateway Operator
You have access to 224 MCP servers and 134+ tools through the Docker MCP Gateway.
Available servers include: GitHub, Playwright, Memory, YouTube, and 220 more.
Complete tasks using any available MCP gateway tools.
Signal completion with: READY_FOR_VALIDATION
"""

DEFAULT_QA_VALIDATOR_PROMPT = """ROLE: QA Validator
Verify MCP Gateway task completion.
...
def init_model_client(task: str = "") -> OpenAIChatCompletionClient:
    ...
    return shared_init_model_client("mcp-gateway", task)


# ========== MCP Gateway Agent ==========
class MCPGatewayAgent:
    """AutoGen agent for Docker MCP Gateway operations (224 servers, 134+ tools)."""
    ...
    async def initialize(self) -> None:
        """Initialize the Docker MCP Gateway workbench and agent."""
        if self._initialized:
            return

        # Prepare environment variables for Docker MCP Gateway
        env = os.environ.copy()

        # Add Windows environment variables for Docker MCP Gateway
        env.update({
        ...
        # Create MCP Gateway server session (directly connect to docker mcp gateway run)
        ...
        if console:
            console.print(f"[green]MCP Gateway Agent initialized with Society of Mind (Operator + QA Validator)[/green]")
            console.print(f"[blue]224 MCP servers, 134+ tools available through Docker MCP Gateway[/blue]")
    ...
    async def run_task(self, task: str, correlation_id: str = None) -> Dict[str, Any]:
        """Execute a task using Docker MCP Gateway tools."""
        ...
        try:
            ...
            async with McpWorkbench(self.server_params) as mcp:
                ...
                operator_prompt = load_prompt_from_module("gateway_operator_prompt", DEFAULT_GITHUB_OPERATOR_PROMPT)
                qa_prompt = load_prompt_from_module("qa_validator_prompt", DEFAULT_QA_VALIDATOR_PROMPT)

                # Create MCP Gateway Operator agent (with workbench)
                gateway_operator = AssistantAgent(
                    "GatewayOperator",
                ...
                print(f"\n{'='*60}")
                print(f"🎭 Society of Mind: MCP Gateway Operator + QA Validator")
                print(f"{'='*60}\n")
...
# ========== Main Entry Point ==========
async def main():
    """Example usage of MCP Gateway agent."""
    agent = MCPGatewayAgent()
    await agent.initialize()

    # Example task
    task = "List the installed MCP servers and available tools"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Docker MCP Gateway Agent with Society of Mind")
