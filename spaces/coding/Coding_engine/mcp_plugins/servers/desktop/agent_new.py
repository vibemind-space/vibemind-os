...
Complete GitHub-related tasks using available GitHub MCP tools.
Signal completion with: READY_FOR_VALIDATION
"""

DEFAULT_QA_VALIDATOR_PROMPT = """ROLE: QA Validator
Verify GitHub task completion.
...
def init_model_client() -> OpenAIChatCompletionClient:
    """Initialize OpenAI chat completion client from config."""
    cfg = load_model_config()
    api_key = os.getenv(cfg.get("api_key_env", "OPENAI_API_KEY"))
    if not api_key:
        raise ValueError(f"Missing API key: {cfg.get('api_key_env')}")
...
class GitHubAgent:
    ...
    async def initialize(self) -> None:
        ...
        if console:
            console.print(f"[green]GitHub Agent initialized with Society of Mind (Operator + QA Validator)[/green]")
