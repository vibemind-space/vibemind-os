...
try:
    import dotenv
    # Path: mcp_plugins/servers/redis/agent.py -> go up 3 levels to project root
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", default="Complete redis task", help="Task for the agent to execute")
    parser.add_argument("--session-id", dest="session_id", help="Session identifier")
    parser.add_argument("--name", default="redis-session", help="Agent session name")
    ...
    parser.add_argument("--grpc-port", dest="grpc_port", type=int, default=50066, help="gRPC port (default: 50066)")
    ...
    if args.grpc:
        ...
        grpc_config = AgentGRPCConfig(
            name="redis",
            port=args.grpc_port,
            agent_runner=run_redis_agent,
            config_class=RedisAgentConfig,
            description="Redis MCP Agent for key-value operations",
            capabilities=["get", "set", "delete", "keys", "publish", "subscribe"]
        )
        print(f"🔴 Starting redis gRPC worker on port {args.grpc_port}...")
        asyncio.run(serve_as_grpc(grpc_config))
    else:
        # Standard CLI Mode
        # Generate session_id if not provided
        session_id = args.session_id or f"redis_{uuid.uuid4().hex[:8]}"
