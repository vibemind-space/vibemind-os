...
class TestBasic:
    def test_returns_id(self):
        s = AgentWorkflowTrackerV2()
        rid = s.track_v2("v1", "v2")
        assert rid.startswith("awtrv-")
...
class TestList:
    ...
    def test_filter(self):
        ...
        s.track_v2("v1", "v2")
        s.track_v2("v3", "v4")
        assert len(s.get_trackings(agent_id="v1")) == 1
    def test_newest_first(self):
        s = AgentWorkflowTrackerV2()
        s.track_v2("v1", "w1")
        s.track_v2("v1", "w2")
        items = s.get_trackings(agent_id="v1")
...
class TestCount:
    ...
    def test_filtered(self):
        ...
        s.track_v2("v1", "v2")
        s.track_v2("v3", "v4")
        assert s.get_tracking_count("v1") == 1
class TestStats:
    def test_data(self):
        s = AgentWorkflowTrackerV2()
        s.track_v2("v1", "v2")
        assert s.get_stats()["total_trackings"] == 1
class TestCallbacks:
...
class TestFireData:
    def test_fire_data_contains_action_key(self):
        ...
        assert captured[0]["record_id"].startswith("awtrv-")
class TestReset:
