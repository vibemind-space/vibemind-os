...
class TestBasic:
    def test_returns_id(self):
        s = PipelineStepSorterV2()
        rid = s.sort_v2("v1", "v2")
        assert rid.startswith("pssov-")
    def test_fields(self):
        s = PipelineStepSorterV2()
        rid = s.sort_v2("v1", "v2", metadata={"k": "v"})
        e = s.get_sort(rid)
    ...
    def test_metadata_deepcopy(self):
        s = PipelineStepSorterV2()
        m = {"x": [1]}
        rid = s.sort_v2("v1", "v2", metadata=m)
        m["x"].append(2)
...
class TestStats:
    def test_data(self):
        s = PipelineStepSorterV2()
        s.sort_v2("v1", "v2")
        assert s.get_stats()["total_sorts"] == 1
class TestCallbacks:
...
class TestFireData:
    def test_fire_data_contains_action_key(self):
        ...
        s.sort_v2("t1", "a1")
        assert captured[0]["action"] == "sort_v2"
        assert captured[0]["record_id"].startswith("pssov-")
class TestReset:
