"""Test mcmp_background — compile and structure verification."""
import py_compile
import sys
sys.path.insert(0, ".")

SOURCE = "src/services/mcmp_background.py"
...
def test_structure():
    ...
    with open(SOURCE, encoding="utf-8") as f:
        src = f.read()
    assert "class MCMPBackgroundSimulation" in src
    assert "class SimulationConfig" in src
    assert "class JudgeMode" in src
    assert "class JudgeResult" in src
