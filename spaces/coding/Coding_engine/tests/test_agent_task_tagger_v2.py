...
class TestBasic:
    ...
    def test_default_param(self):
        s = AgentTaskTaggerV2()
        rid = s.tag_v2("v1", "v2")
        assert s.get_tag(rid)["label"] == "default"
...
class TestList:
    ...
    def test_filter(self):
        ...
        s.tag_v2("v1", "v2")
        s.tag_v2("v3", "v4")
        assert len(s.get_tags(agent_id="v2")) == 1
    def test_newest_first(self):
        s = AgentTaskTaggerV2()
        s.tag_v2("t1", "a1")
        s.tag_v2("t2", "a1")
        items = s.get_tags(agent_id="a1")
...
class TestCount:
    ...
    def test_filtered(self):
        ...
        s.tag_v2("v1", "v2")
        s.tag_v2("v3", "v4")
        assert s.get_tag_count("v2") == 1
class TestStats:
    def test_data(self):
        s = AgentTaskTaggerV2()
        s.tag_v2("v1", "v2")
        assert s.get_stats()["total_tags"] == 1
class TestCallbacks:
...
class TestFireData:
    def test_fire_data_contains_action_key(self):
        ...
        assert captured[0]["record_id"].startswith("attv-")
class TestReset:
