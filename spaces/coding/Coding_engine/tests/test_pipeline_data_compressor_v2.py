...
class TestBasic:
    def test_returns_id(self):
        s = PipelineDataCompressorV2()
        rid = s.compress_v2("v1", "v2")
        assert rid.startswith("pdcpv-")
    def test_fields(self):
        s = PipelineDataCompressorV2()
        rid = s.compress_v2("v1", "v2", metadata={"k": "v"})
    ...
    def test_metadata_deepcopy(self):
        s = PipelineDataCompressorV2()
        m = {"x": [1]}
        rid = s.compress_v2("v1", "v2", metadata=m)
        m["x"].append(2)
        assert s.get_compression(rid)["metadata"]["x"] == [1]
    def test_empty_p1(self):
        s = PipelineDataCompressorV2()
        assert s.compress_v2("", "v2") == ""
    def test_empty_p2(self):
        s = PipelineDataCompressorV2()
        assert s.compress_v2("v1", "") == ""
class TestGet:
    def test_found(self):
        s = PipelineDataCompressorV2()
        rid = s.compress_v2("v1", "v2")
...
class TestStats:
    def test_data(self):
        s = PipelineDataCompressorV2()
        s.compress_v2("v1", "v2")
        assert s.get_stats()["total_compressions"] == 1
class TestCallbacks:
...
class TestFireData:
    def test_fire_data_contains_action_key(self):
        ...
        s.compress_v2("t1", "a1")
        assert captured[0]["action"] == "compress_v2"
        assert captured[0]["record_id"].startswith("pdcpv-")
class TestReset:
