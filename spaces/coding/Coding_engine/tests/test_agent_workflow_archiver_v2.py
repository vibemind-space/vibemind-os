...
class TestBasic:
    def test_returns_id(self):
        s = AgentWorkflowArchiverV2()
        rid = s.archive_v2("v1", "v2")
        assert rid.startswith("awav-")
    ...
    def test_default_param(self):
        s = AgentWorkflowArchiverV2()
        rid = s.archive_v2("v1", "v2")
        assert s.get_archive(rid)["destination"] == "cold"
...
class TestGet:
    ...
    def test_copy(self):
        s = AgentWorkflowArchiverV2()
        rid = s.archive_v2("v1", "v2")
        e1 = s.get_archive(rid)
        e2 = s.get_archive(rid)
        assert e1 is not e2
...
class TestList:
    ...
    def test_filter(self):
        ...
        s.archive_v2("v1", "v2")
        s.archive_v2("v3", "v4")
        assert len(s.get_archives(agent_id="v1")) == 1

    def test_newest_first(self):
        s = AgentWorkflowArchiverV2()
        s.archive_v2("v1", "a1")
        s.archive_v2("v1", "a2")
        items = s.get_archives(agent_id="v1")
