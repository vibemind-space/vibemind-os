...
@dataclass
class PipelineStepReordererV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class PipelineStepReordererV2:
    PREFIX = "psrov-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineStepReordererV2State()
    ...
    def reorder_v2(self, pipeline_id: str, step_name: str, position: int = 1, metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("reorder_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_reorders": len(entries), "unique_pipelines": len(unique)}
