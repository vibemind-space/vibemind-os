...
@dataclass
class AgentTaskEscalationV2State:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentTaskEscalationV2:
    PREFIX = "atev-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = AgentTaskEscalationV2State()
    ...
    def get_escalations(self, agent_id: str = "", limit: int = 50) -> List[dict]:
        if agent_id:
    ...
    def get_escalation_count(self, agent_id: str = "") -> int:
        if agent_id:
