"""Service module for throttling pipeline data operations."""
...
@dataclass
class PipelineDataThrottlerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataThrottler:
    """Throttles pipeline data operations."""

    PREFIX = "pdth-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataThrottlerState()
    ...
    def throttle(
        ...
        """Throttle a pipeline data operation and return the record ID."""
        ...
        self._prune()
        self._fire("throttled", record)
        return record_id

    def get_throttle(self, record_id: str) -> Optional[dict]:
        """Return the throttle record for *record_id*, or ``None``."""
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_throttles(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        """Return throttles, newest first, optionally filtered by *pipeline_id*."""
    ...
    def get_throttle_count(self, pipeline_id: str = "") -> int:
        """Return the number of throttles, optionally filtered by *pipeline_id*."""
        if not pipeline_id:
            return len(self._state.entries)
        return sum(1 for e in self._state.entries.values() if e.get("pipeline_id") == pipeline_id)

    def get_stats(self) -> dict:
        """Return summary statistics about stored throttles."""
        all_pipelines = set()
    ...
    def reset(self) -> None:
        """Clear all throttles and reset state."""
        self._state = PipelineDataThrottlerState()
