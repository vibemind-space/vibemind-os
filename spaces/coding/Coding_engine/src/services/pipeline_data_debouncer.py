"""Service module for debouncing pipeline data operations."""
...
@dataclass
class PipelineDataDebouncerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataDebouncer:
    """Debounces pipeline data operations."""

    PREFIX = "pddb-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataDebouncerState()
    ...
    def debounce(
        ...
        """Debounce a pipeline data operation and return the record ID."""
        ...
        self._prune()
        self._fire("debounced", record)
        return record_id

    def get_debounce(self, record_id: str) -> Optional[dict]:
        """Return the debounce record for *record_id*, or ``None``."""
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_debounces(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        """Return debounces, newest first, optionally filtered by *pipeline_id*."""
    ...
    def get_debounce_count(self, pipeline_id: str = "") -> int:
        """Return the number of debounces, optionally filtered by *pipeline_id*."""
        if not pipeline_id:
            return len(self._state.entries)
        return sum(1 for e in self._state.entries.values() if e.get("pipeline_id") == pipeline_id)

    def get_stats(self) -> dict:
        """Return summary statistics about stored debounces."""
        all_pipelines = set()
    ...
    def reset(self) -> None:
        """Clear all debounces and reset state."""
        self._state = PipelineDataDebouncerState()
