...
@dataclass
class AgentTaskVerifierState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentTaskVerifier:
    PREFIX = "atve-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = AgentTaskVerifierState()
    ...
    def verify(self, task_id: str, agent_id: str, verdict: str = "passed", metadata: Optional[dict] = None) -> str:
        if not task_id or not agent_id:
            return ""
        record_id = self._generate_id()
        entry = {"record_id": record_id, "task_id": task_id, "agent_id": agent_id, "verdict": verdict, "metadata": copy.deepcopy(metadata) if metadata else {}, "created_at": time.time(), "_seq": self._state._seq}
        self._state.entries[record_id] = entry
        self._prune()
        self._fire("verified", entry)
        return record_id

    def get_verification(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_verifications(self, agent_id: str = "", limit: int = 50) -> List[dict]:
        if agent_id:
    ...
    def get_verification_count(self, agent_id: str = "") -> int:
        if agent_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_verifications": len(self._state.entries), "unique_agents": len(unique_agents)}
