"""Service module for dispatching pipeline data to named targets."""
...
@dataclass
class PipelineDataDispatcherState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0


class PipelineDataDispatcher:
    """Dispatches pipeline data to named targets."""

    PREFIX = "pddi-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataDispatcherState()
    ...
    def dispatch(
        ...
        """Dispatch data to a named target and return the dispatch record ID."""
        ...
        self._prune()
        self._fire("dispatch", record)
        return record_id

    def get_dispatch(self, record_id: str) -> Optional[dict]:
        """Return the dispatch record for *record_id*, or ``None``."""
    ...
    def get_dispatches(
        self, pipeline_id: str = "", target: str = "", limit: int = 50
    ) -> List[dict]:
        """Return dispatch records, newest first, optionally filtered."""
    ...
    def get_dispatch_count(self, pipeline_id: str = "") -> int:
        """Return the number of dispatch records, optionally filtered by *pipeline_id*."""
        if not pipeline_id:
    ...
    def get_stats(self) -> dict:
        """Return summary statistics about dispatched records."""
        targets = {e.get("target", "") for e in self._state.entries.values()}
