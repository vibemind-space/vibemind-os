"""Agent task notifier v2 -- sends notifications for agent task events."""
...
@dataclass
class AgentTaskNotifierV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class AgentTaskNotifierV2:
    PREFIX = "atnv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentTaskNotifierV2State()
    ...
    def notify_v2(self, task_id: str, agent_id: str, channel: str = "default", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("notify_v2", record_id=record_id, task_id=task_id)
        return record_id

    def get_notification(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_notifications(self, agent_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_notifications": len(entries), "unique_agents": len(agents)}
