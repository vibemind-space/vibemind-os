"""Agent Config Store – stores agent configuration as key-value pairs.
...
@dataclass
class _StoreState:
    entries: Dict[str, _ConfigEntry] = field(default_factory=dict)
...
class AgentConfigStore:
    ...
    prefix 'acs-'. Entries are indexed by agent_id and key for fast lookup.
    """

    def __init__(self, max_entries: int = 10000) -> None:
        self._state = _StoreState(max_entries=max_entries)
        logger.info("agent_config_store.initialized", max_entries=max_entries)
    ...
    def on_change(self, name: str, callback: Callable) -> None:
        ...
        logger.debug("agent_config_store.callback_registered", name=name)

    def set(self, agent_id: str, namespace: str, key: str, value: Any) -> str:
        """Backward-compatible: set(agent_id, namespace, key, value)."""
        composite_key = f"{namespace}.{key}"
        return self.set_config(agent_id, composite_key, value)

    def get(self, agent_id: str, namespace: str, key: str, default: Any = None) -> Any:
        """Backward-compatible: get(agent_id, namespace, key, default=None)."""
        composite_key = f"{namespace}.{key}"
        return self.get_config(agent_id, composite_key, default=default)

    def remove_callback(self, name: str) -> bool:
        """Remove a registered callback."""
        if name in self._state.callbacks:
            del self._state.callbacks[name]
            logger.debug("agent_config_store.callback_removed", name=name)
            return True
        return False
    ...
    def _fire(self, event: str, data: Dict[str, Any]) -> None:
        ...
        for name, cb in list(self._state.callbacks.items()):
            try:
                ...
                logger.exception(
                    "agent_config_store.callback_error",
                    callback_name=name,
                    event=event,
                )
    ...
    def _prune_if_needed(self) -> None:
        ...
        if len(self._state.entries) <= self._state.max_entries:
            return
        ...
        to_remove = len(self._state.entries) - self._state.max_entries
        for entry in sorted_entries[:to_remove]:
            del self._state.entries[entry.config_id]
        ...
        logger.debug("agent_config_store.pruned", removed=to_remove)
    ...
    def set_config(self, agent_id: str, key: str, value: Any) -> str:
        ...
        Returns:
            The config_id (prefix 'acs-') of the stored entry.
        """
        now = time.time()
        ...
        if existing_id and existing_id in self._state.entries:
            entry = self._state.entries[existing_id]
    ...
    def get_config(self, agent_id: str, key: str, default: Any = None) -> Any:
        ...
        if config_id is None or config_id not in self._state.entries:
            return default
        return self._state.entries[config_id].value
    ...
    def get_all_config(self, agent_id: str) -> Dict[str, Any]:
        ...
        for key, config_id in agent_keys.items():
            if config_id in self._state.entries:
                result[key] = self._state.entries[config_id].value
        return result
    ...
    def delete_config(self, agent_id: str, key: str) -> bool:
        ...
        entry = self._state.entries.pop(config_id)
    ...
    def has_config(self, agent_id: str, key: str) -> bool:
        """Check if a configuration entry exists for an agent.
        ...
        """
        config_id = self._state.agent_index.get(agent_id, {}).get(key)
        return config_id is not None and config_id in self._state.entries
    ...
    def reset(self) -> None:
        ...
        logger.info("agent_config_store.reset")
