...
@dataclass
class PipelineStepRerunnerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineStepRerunner:
    PREFIX = "psrr-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = PipelineStepRerunnerState()
    ...
    def rerun(self, pipeline_id: str, step_name: str, attempt: int = 1, metadata: Optional[dict] = None) -> str:
        if not pipeline_id or not step_name:
            return ""
        record_id = self._generate_id()
        entry = {"record_id": record_id, "pipeline_id": pipeline_id, "step_name": step_name, "attempt": attempt, "metadata": copy.deepcopy(metadata) if metadata else {}, "created_at": time.time(), "_seq": self._state._seq}
        self._state.entries[record_id] = entry
        self._prune()
        self._fire("rerun", entry)
        return record_id

    def get_rerun(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_reruns(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        if pipeline_id:
    ...
    def get_rerun_count(self, pipeline_id: str = "") -> int:
        if pipeline_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_reruns": len(self._state.entries), "unique_pipelines": len(unique_pipelines)}
