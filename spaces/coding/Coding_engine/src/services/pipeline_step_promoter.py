...
@dataclass
class PipelineStepPromoterState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineStepPromoter:
    PREFIX = "pspm-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = PipelineStepPromoterState()
    ...
    def promote(
        ...
        self._prune()
        self._fire("promoted", entry)
        return record_id

    def get_promotion(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_promotions(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        if pipeline_id:
    ...
    def get_promotion_count(self, pipeline_id: str = "") -> int:
        if pipeline_id:
