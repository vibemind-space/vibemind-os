...
@dataclass
class PipelineStepMigratorState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineStepMigrator:
    PREFIX = "psmr-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = PipelineStepMigratorState()
    ...
    def migrate(self, pipeline_id: str, step_name: str, target_env: str = "staging", metadata: Optional[dict] = None) -> str:
        if not pipeline_id or not step_name:
            return ""
        record_id = self._generate_id()
        entry = {"record_id": record_id, "pipeline_id": pipeline_id, "step_name": step_name, "target_env": target_env, "metadata": copy.deepcopy(metadata) if metadata else {}, "created_at": time.time(), "_seq": self._state._seq}
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_migrations": len(self._state.entries), "unique_pipelines": len(unique_pipelines)}
