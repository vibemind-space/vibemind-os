"""Pipeline step executor v2 service."""
...
@dataclass
class PipelineStepExecutorV2State:
    """State container for PipelineStepExecutorV2."""
...
class PipelineStepExecutorV2:
    """Executes pipeline steps and tracks execution records."""

    PREFIX = "psev-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineStepExecutorV2State()
    ...
    def execute_v2(
        ...
        self._prune()
        self._fire("execute_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id

    def get_execution(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return deepcopy(entry)

    def get_executions(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        if pipeline_id:
            items = [
                e
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_executions": len(self._state.entries),
