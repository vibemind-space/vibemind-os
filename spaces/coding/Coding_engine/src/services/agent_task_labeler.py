...
@dataclass
class AgentTaskLabelerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentTaskLabeler:
    PREFIX = "atlb-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = AgentTaskLabelerState()
    ...
    def label(self, task_id: str, agent_id: str, label_name: str = "", metadata: Optional[dict] = None) -> str:
        if not task_id or not agent_id:
            return ""
        record_id = self._generate_id()
        entry = {"record_id": record_id, "task_id": task_id, "agent_id": agent_id, "label_name": label_name, "metadata": copy.deepcopy(metadata) if metadata else {}, "created_at": time.time(), "_seq": self._state._seq}
        self._state.entries[record_id] = entry
        self._prune()
        self._fire("labeled", entry)
        return record_id
    ...
    def get_labels(self, agent_id: str = "", limit: int = 50) -> List[dict]:
        if agent_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_labels": len(self._state.entries), "unique_agents": len(unique_agents)}
