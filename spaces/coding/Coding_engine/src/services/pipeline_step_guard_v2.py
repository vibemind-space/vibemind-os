...
@dataclass
class PipelineStepGuardV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class PipelineStepGuardV2:
    PREFIX = "psgv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineStepGuardV2State()
    ...
    def guard_v2(self, pipeline_id: str, step_name: str, condition: str = "always", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("guard_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id

    def get_guard(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_guards(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_guards": len(entries), "unique_pipelines": len(unique)}
