"""Service module for prefetching pipeline data."""
...
@dataclass
class PipelineDataPrefetcherState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataPrefetcher:
    """Prefetches pipeline data."""

    PREFIX = "pdpf-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataPrefetcherState()
    ...
    def prefetch(self, pipeline_id: str, data_key: str, priority: int = 0, metadata: Optional[dict] = None) -> str:
        """Prefetch pipeline data and return the record ID."""
        ...
        self._prune()
        self._fire("prefetched", entry)
        return record_id

    def get_prefetch(self, record_id: str) -> Optional[dict]:
        """Return the prefetch record for *record_id*, or ``None``."""
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_prefetches(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        """Return prefetch records, newest first, optionally filtered by *pipeline_id*."""
    ...
    def get_prefetch_count(self, pipeline_id: str = "") -> int:
        """Return the number of prefetches, optionally filtered by *pipeline_id*."""
        if not pipeline_id:
            return len(self._state.entries)
        return sum(1 for e in self._state.entries.values() if e.get("pipeline_id") == pipeline_id)

    def get_stats(self) -> dict:
        """Return summary statistics about stored prefetches."""
        pipeline_ids = {e.get("pipeline_id", "") for e in self._state.entries.values()}
        return {
            "total_prefetches": len(self._state.entries),
            "unique_pipelines": len(pipeline_ids),
        }

    def reset(self) -> None:
        """Clear all prefetch records, callbacks, and on_change."""
        self._state = PipelineDataPrefetcherState()
