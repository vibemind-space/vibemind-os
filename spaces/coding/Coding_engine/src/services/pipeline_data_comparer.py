...
@dataclass
class PipelineDataComparerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataComparer:
    PREFIX = "pdcp-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = PipelineDataComparerState()
    ...
    def compare(self, pipeline_id: str, data_key_a: str, data_key_b: str, mode: str = "diff", metadata: Optional[dict] = None) -> str:
        if not pipeline_id or not data_key_a or not data_key_b:
            return ""
        record_id = self._generate_id()
        entry = {"record_id": record_id, "pipeline_id": pipeline_id, "data_key_a": data_key_a, "data_key_b": data_key_b, "mode": mode, "metadata": copy.deepcopy(metadata) if metadata else {}, "created_at": time.time(), "_seq": self._state._seq}
        self._state.entries[record_id] = entry
        self._prune()
        self._fire("compared", entry)
        return record_id

    def get_comparison(self, record_id: str) -> Optional[dict]:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_comparisons": len(self._state.entries), "unique_pipelines": len(unique_pipelines)}
