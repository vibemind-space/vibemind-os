...
@dataclass
class AgentWorkflowReplayerV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class AgentWorkflowReplayerV2:
    PREFIX = "awrv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentWorkflowReplayerV2State()
    ...
    def replay_v2(self, agent_id: str, workflow_name: str, speed: str = "1x", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("replay_v2", agent_id=agent_id, record_id=record_id)
        return record_id

    def get_replay(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_replays(self, agent_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_replays": len(entries), "unique_agents": len(unique)}
