"""Agent Task Completer V2 -- completes tasks for agents.
...
Usage::
    ...
    completer = AgentTaskCompleterV2()

    # Complete a task
    record_id = completer.complete_v2("task-1", "agent-1", status="done")
...
@dataclass
class AgentTaskCompleterV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentTaskCompleterV2:
    """Completes tasks for agents (v2)."""

    PREFIX = "atcv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentTaskCompleterV2State()
    ...
    def complete_v2(
        ...
        self._prune()
        self._fire("complete_v2", record_id=record_id, task_id=task_id, agent_id=agent_id)
        logger.debug(
    ...
    def reset(self) -> None:
        ...
        logger.debug("AgentTaskCompleterV2 reset")
