...
@dataclass
class AgentWorkflowCompilerV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class AgentWorkflowCompilerV2:
    PREFIX = "awcpv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentWorkflowCompilerV2State()
    ...
    def compile_wf_v2(self, agent_id: str, workflow_name: str, target: str = "bytecode", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("compile_wf_v2", agent_id=agent_id, record_id=record_id)
        return record_id
    ...
    def get_compilations(self, agent_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_compilation_count(self, agent_id: str = "") -> int:
        if not agent_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_compilations": len(entries), "unique_agents": len(unique)}
