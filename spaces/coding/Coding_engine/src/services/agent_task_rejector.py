...
@dataclass
class AgentTaskRejectorState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentTaskRejector:
    PREFIX = "atrj-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = AgentTaskRejectorState()
    ...
    def reject(
        ...
        self._prune()
        self._fire("rejected", entry)
        return record_id

    def get_rejection(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_rejections(self, agent_id: str = "", limit: int = 50) -> List[dict]:
        if agent_id:
    ...
    def get_rejection_count(self, agent_id: str = "") -> int:
        if agent_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_rejections": len(self._state.entries),
