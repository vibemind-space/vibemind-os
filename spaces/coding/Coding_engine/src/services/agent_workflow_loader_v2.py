...
@dataclass
class AgentWorkflowLoaderV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class AgentWorkflowLoaderV2:
    PREFIX = "awldv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentWorkflowLoaderV2State()
    ...
    def load_v2(self, agent_id: str, workflow_name: str, source: str = "disk", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("load_v2", agent_id=agent_id, record_id=record_id)
        return record_id

    def get_load(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_loads(self, agent_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_load_count(self, agent_id: str = "") -> int:
        if not agent_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_loads": len(entries), "unique_agents": len(unique)}
