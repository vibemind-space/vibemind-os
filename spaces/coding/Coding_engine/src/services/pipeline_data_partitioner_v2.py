...
@dataclass
class PipelineDataPartitionerV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class PipelineDataPartitionerV2:
    PREFIX = "pdptv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataPartitionerV2State()
    ...
    def partition_v2(self, pipeline_id: str, data_key: str, count: int = 4, metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("partition_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_partitions": len(entries), "unique_pipelines": len(unique)}
