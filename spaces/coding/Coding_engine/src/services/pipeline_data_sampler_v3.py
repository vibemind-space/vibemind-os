...
@dataclass
class PipelineDataSamplerV3State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class PipelineDataSamplerV3:
    PREFIX = "pds3-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataSamplerV3State()
    ...
    def sample_v3(self, pipeline_id: str, data_key: str, ratio: str = "0.1", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("sample_v3", pipeline_id=pipeline_id, record_id=record_id)
        return record_id

    def get_sample(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_samples(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_samples": len(entries), "unique_pipelines": len(unique)}
