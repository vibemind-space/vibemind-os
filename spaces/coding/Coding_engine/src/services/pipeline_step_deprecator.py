"""Pipeline step deprecator -- deprecates pipeline steps.

Stores deprecation records that associate pipeline steps with a reason
...
@dataclass
class PipelineStepDeprecatorState:
    """Internal mutable state for the deprecator service."""
...
class PipelineStepDeprecator:
    """Deprecates pipeline steps.

    Each deprecation record ties a pipeline step to a reason string
    ...
    def __init__(self) -> None:
        self._state = PipelineStepDeprecatorState()
    ...
    def _generate_id(self, key: str) -> str:
        self._state._seq += 1
        raw = f"{self.PREFIX}{self._state._seq}-{key}"
    ...
    def deprecate(
        ...
        """Deprecate a pipeline step. Returns the record ID.
        ...
        self._prune()
        self._fire("deprecate", record_id=record_id, pipeline_id=pipeline_id, step_name=step_name)
        return record_id
    ...
    def get_deprecation(self, record_id: str) -> Optional[dict]:
        """Return a copy of a single deprecation record, or None."""
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_deprecations(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        """Return deprecation records, newest first.
    ...
    def get_deprecation_count(self, pipeline_id: str = "") -> int:
        """Return the number of deprecation records.
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_deprecations": len(self._state.entries),
