...
@dataclass
class AgentWorkflowPrioritizerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentWorkflowPrioritizer:
    PREFIX = "awpr-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = AgentWorkflowPrioritizerState()
    ...
    def prioritize(self, agent_id: str, workflow_name: str, priority: int = 5, metadata: Optional[dict] = None) -> str:
        if not agent_id or not workflow_name:
            return ""
        record_id = self._generate_id()
        entry = {"record_id": record_id, "agent_id": agent_id, "workflow_name": workflow_name, "priority": priority, "metadata": copy.deepcopy(metadata) if metadata else {}, "created_at": time.time(), "_seq": self._state._seq}
        self._state.entries[record_id] = entry
        self._prune()
        self._fire("prioritized", entry)
        return record_id
    ...
    def get_priorities(self, agent_id: str = "", limit: int = 50) -> List[dict]:
        if agent_id:
    ...
    def get_priority_count(self, agent_id: str = "") -> int:
        if agent_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_priorities": len(self._state.entries), "unique_agents": len(unique_agents)}
