...
@dataclass
class AgentWorkflowResumerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentWorkflowResumer:
    PREFIX = "awrs-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = AgentWorkflowResumerState()
    ...
    def get_resumptions(self, agent_id: str = "", limit: int = 50) -> List[dict]:
        if agent_id:
