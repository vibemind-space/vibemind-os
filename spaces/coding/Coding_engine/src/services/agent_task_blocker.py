"""Agent Task Blocker – blocks tasks for agents with tracking and callbacks."""
...
@dataclass
class AgentTaskBlockerState:
    """State container for the agent task blocker."""
...
class AgentTaskBlocker:
    """Blocks tasks for agents, tracking block records with metadata."""

    PREFIX = "atbl-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentTaskBlockerState()
    ...
    def block(
        ...
        """Block a task for an agent. Returns the record_id or '' on invalid input."""
        ...
        self._prune()
        self._fire("blocked", self._state.entries[record_id])
        logger.debug("Blocked task %s for agent %s: %s", task_id, agent_id, record_id)
        return record_id

    def get_block(self, record_id: str) -> dict:
        """Return the block entry or empty dict if not found."""
        entry = self._state.entries.get(record_id)
        if entry is None:
            return {}
        return copy.deepcopy(entry)

    def get_blocks(self, agent_id: str = "") -> list:
        """Return all blocks, optionally filtered by agent_id."""
    ...
    def get_block_count(self, agent_id: str = "") -> int:
        """Return the number of blocks, optionally filtered by agent_id."""
        if not agent_id:
    ...
    def reset(self) -> None:
        ...
        logger.debug("AgentTaskBlocker reset")
