...
@dataclass
class AgentTaskCancelerV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class AgentTaskCancelerV2:
    PREFIX = "atcv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentTaskCancelerV2State()
    ...
    def cancel_v2(self, task_id: str, agent_id: str, reason: str = "timeout", metadata: Optional[dict] = None) -> str:
    ...
    def get_cancel(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_cancels(self, agent_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_cancel_count(self, agent_id: str = "") -> int:
        if not agent_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_cancels": len(entries), "unique_agents": len(unique)}
