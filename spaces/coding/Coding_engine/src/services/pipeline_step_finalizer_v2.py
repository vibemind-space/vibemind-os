...
@dataclass
class PipelineStepFinalizerV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class PipelineStepFinalizerV2:
    PREFIX = "psfv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineStepFinalizerV2State()
    ...
    def finalize_v2(self, pipeline_id: str, step_name: str, status: str = "complete", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("finalize_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id
    ...
    def get_finalizations(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_finalization_count(self, pipeline_id: str = "") -> int:
        if not pipeline_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_finalizations": len(entries), "unique_pipelines": len(unique)}
