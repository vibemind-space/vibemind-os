...
@dataclass
class AgentTaskGrouperV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class AgentTaskGrouperV2:
    PREFIX = "atgv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentTaskGrouperV2State()
    ...
    def group_v2(self, task_id: str, agent_id: str, category: str = "default", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("group_v2", task_id=task_id, record_id=record_id)
        return record_id

    def get_group(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_groups(self, agent_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_groups": len(entries), "unique_agents": len(unique)}
