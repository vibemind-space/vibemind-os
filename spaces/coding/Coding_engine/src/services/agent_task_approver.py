...
@dataclass
class AgentTaskApproverState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentTaskApprover:
    PREFIX = "atap-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = AgentTaskApproverState()
    ...
    def approve(self, task_id: str, agent_id: str, decision: str = "approved", metadata: Optional[dict] = None) -> str:
        if not task_id or not agent_id:
            return ""
        record_id = self._generate_id()
        entry = {"record_id": record_id, "task_id": task_id, "agent_id": agent_id, "decision": decision, "metadata": copy.deepcopy(metadata) if metadata else {}, "created_at": time.time(), "_seq": self._state._seq}
        self._state.entries[record_id] = entry
        self._prune()
        self._fire("approved", entry)
        return record_id

    def get_approval(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_approvals(self, agent_id: str = "", limit: int = 50) -> List[dict]:
        if agent_id:
    ...
    def get_approval_count(self, agent_id: str = "") -> int:
        if agent_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_approvals": len(self._state.entries), "unique_agents": len(unique_agents)}
