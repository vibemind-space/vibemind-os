"""Service module for packing pipeline data."""
...
@dataclass
class PipelineDataPackerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataPacker:
    """Packs pipeline data."""
    ...
    def __init__(self) -> None:
        self._state = PipelineDataPackerState()
    ...
    def pack(
        ...
        """Pack pipeline data and return the record ID."""
        ...
        self._prune()
        self._fire("packed", record)
        return record_id

    def get_pack(self, record_id: str) -> Optional[dict]:
        """Return the pack record for *record_id*, or ``None``."""
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_packs(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        """Return packs, newest first, optionally filtered by *pipeline_id*."""
    ...
    def get_pack_count(self, pipeline_id: str = "") -> int:
        """Return the number of packs, optionally filtered by *pipeline_id*."""
        if not pipeline_id:
            return len(self._state.entries)
        return sum(1 for e in self._state.entries.values() if e.get("pipeline_id") == pipeline_id)

    def get_stats(self) -> dict:
        """Return summary statistics about stored packs."""
        all_pipelines = set()
        for e in self._state.entries.values():
            all_pipelines.add(e.get("pipeline_id", ""))
        return {
            "total_packs": len(self._state.entries),
