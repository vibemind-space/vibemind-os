"""Agent Task Pauser -- pauses tasks for agents.
...
Usage::
    ...
    pauser = AgentTaskPauser()

    # Pause a task
    record_id = pauser.pause("task-1", "agent-1", reason="maintenance")
...
@dataclass
class AgentTaskPauserState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentTaskPauser:
    """Pauses tasks for agents."""

    PREFIX = "atpa-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentTaskPauserState()
    ...
    def pause(
        ...
        self._fire("paused", self._state.entries[record_id])
        logger.debug(
    ...
    def reset(self) -> None:
        ...
        logger.debug("AgentTaskPauser reset")
