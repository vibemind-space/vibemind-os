...
@dataclass
class PipelineDataDequeuerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataDequeuer:
    PREFIX = "pddq-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = PipelineDataDequeuerState()
    ...
    def get_dequeues(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        if pipeline_id:
    ...
    def get_dequeue_count(self, pipeline_id: str = "") -> int:
        if pipeline_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_dequeues": len(self._state.entries),
