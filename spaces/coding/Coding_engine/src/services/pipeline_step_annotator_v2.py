...
@dataclass
class PipelineStepAnnotatorV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class PipelineStepAnnotatorV2:
    PREFIX = "psanv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineStepAnnotatorV2State()
    ...
    def annotate_v2(self, pipeline_id: str, step_name: str, note: str = "none", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("annotate_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id

    def get_annotation(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_annotations(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_annotations": len(entries), "unique_pipelines": len(unique)}
