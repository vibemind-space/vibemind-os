...
@dataclass
class AgentWorkflowScalerV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class AgentWorkflowScalerV2:
    PREFIX = "awsv2-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentWorkflowScalerV2State()
    ...
    def scale_v2(self, agent_id: str, workflow_name: str, factor: int = 1, metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("scale_v2", agent_id=agent_id, record_id=record_id)
        return record_id
    ...
    def get_scales(self, agent_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_scale_count(self, agent_id: str = "") -> int:
        if not agent_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_scales": len(entries), "unique_agents": len(unique)}
