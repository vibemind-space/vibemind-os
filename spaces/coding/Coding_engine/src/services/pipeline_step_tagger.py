"""Pipeline step tagger — tags pipeline steps with labels and categories.

Assigns and manages tag labels on individual pipeline steps, enabling
categorization, filtering, and organization of pipeline workflows.
"""
...
@dataclass
class PipelineStepTaggerState:
    """Internal state for the PipelineStepTagger service."""
...
class PipelineStepTagger:
    """Tags pipeline steps with labels and categories.

    Records tag assignments on pipeline steps, allowing steps to be
    categorized, searched, and filtered by their associated tags.
    """
    ...
    def __init__(self) -> None:
        self._state = PipelineStepTaggerState()
    ...
    def tag(
        ...
        """Record a tag assignment on a pipeline step. Returns record ID."""
        self._prune()
    ...
    def get_tag(self, record_id: str) -> Optional[dict]:
        """Get a single tag record by ID. Returns None if not found."""
    ...
    def get_tags(
        ...
        """Get tag records, newest first. Optionally filter by pipeline_id."""
    ...
    def get_tag_count(self, pipeline_id: str = "") -> int:
        """Count tag records, optionally filtering by pipeline_id."""
        if not pipeline_id:
