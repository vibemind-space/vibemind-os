"""Pipeline step recorder — records pipeline step execution history for replay/audit.

Captures input/output data for each pipeline step execution, enabling
historical replay, debugging, and audit trail capabilities.
"""
...
@dataclass
class PipelineStepRecorderState:
    """Internal state for the PipelineStepRecorder service."""
...
class PipelineStepRecorder:
    ...
    Captures the full input/output context of each pipeline step execution,
    allowing historical analysis, replay, and compliance auditing.
    """
    ...
    def __init__(self) -> None:
        self._state = PipelineStepRecorderState()
    ...
    def record(
        ...
        """Record a pipeline step execution. Returns record ID."""
        self._prune()
        ...
        self._fire("recorded", dict(entry))
        return record_id
    ...
    def get_recording(self, record_id: str) -> Optional[dict]:
        """Get a single recording by ID. Returns None if not found."""
    ...
    def get_recordings(
        ...
        """Get recordings, newest first. Optionally filter by pipeline_id and/or step_name."""
