...
@dataclass
class PipelineStepUnfreezerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineStepUnfreezer:
    PREFIX = "psuf-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = PipelineStepUnfreezerState()
    ...
    def unfreeze(
        ...
        self._prune()
        self._fire("unfrozen", entry)
        return record_id

    def get_unfreeze(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_unfreezes(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        if pipeline_id:
    ...
    def get_unfreeze_count(self, pipeline_id: str = "") -> int:
        if pipeline_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_unfreezes": len(self._state.entries),
