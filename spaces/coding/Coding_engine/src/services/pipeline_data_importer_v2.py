...
@dataclass
class PipelineDataImporterV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataImporterV2:
    PREFIX = "pdiv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataImporterV2State()
    ...
    def import_v2(
        ...
        self._prune()
        self._fire("import_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id

    def get_import(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is not None:
            return copy.deepcopy(entry)
        return None

    def get_imports(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_import_count(self, pipeline_id: str = "") -> int:
        if not pipeline_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_imports": len(self._state.entries),
