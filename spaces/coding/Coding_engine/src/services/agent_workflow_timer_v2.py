...
@dataclass
class AgentWorkflowTimerV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class AgentWorkflowTimerV2:
    PREFIX = "awtv2-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = AgentWorkflowTimerV2State()
    ...
    def time_v2(self, agent_id: str, workflow_name: str, timeout: int = 300, metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("time_v2", agent_id=agent_id, record_id=record_id)
        return record_id

    def get_timer(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_timers(self, agent_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_timers": len(entries), "unique_agents": len(unique)}
