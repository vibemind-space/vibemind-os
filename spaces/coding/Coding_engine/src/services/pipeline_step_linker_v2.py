...
@dataclass
class PipelineStepLinkerV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class PipelineStepLinkerV2:
    PREFIX = "pslv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineStepLinkerV2State()
    ...
    def link_v2(self, pipeline_id: str, step_name: str, target: str = "next", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("link_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_links": len(entries), "unique_pipelines": len(unique)}
