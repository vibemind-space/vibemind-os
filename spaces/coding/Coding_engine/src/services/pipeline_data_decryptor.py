...
@dataclass
class PipelineDataDecryptorState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataDecryptor:
    PREFIX = "pddy-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataDecryptorState()
    ...
    def decrypt(
        ...
        self._prune()
        self._fire("decrypted", entry)
        return record_id

    def get_decryption(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return copy.deepcopy(entry)

    def get_decryptions(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        if pipeline_id:
            items = [
                e
    ...
    def get_decryption_count(self, pipeline_id: str = "") -> int:
        if not pipeline_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_decryptions": len(self._state.entries),
