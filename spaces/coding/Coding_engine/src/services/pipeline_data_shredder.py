"""Service module for securely shredding pipeline data."""
...
@dataclass
class PipelineDataShredderState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataShredder:
    """Shreds/securely deletes pipeline data records."""

    PREFIX = "pdsh-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataShredderState()
    ...
    def shred(
        ...
        """Securely shred pipeline data and return the record ID."""
        ...
        self._prune()
        self._fire("shredded", record)
        return record_id

    def get_shred(self, record_id: str) -> Optional[dict]:
        """Return the shred record for *record_id*, or ``None``."""
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_shreds(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        """Return shreds, newest first, optionally filtered by *pipeline_id*."""
    ...
    def get_shred_count(self, pipeline_id: str = "") -> int:
        """Return the number of shreds, optionally filtered by *pipeline_id*."""
        if not pipeline_id:
            return len(self._state.entries)
        return sum(1 for e in self._state.entries.values() if e.get("pipeline_id") == pipeline_id)

    def get_stats(self) -> dict:
        """Return summary statistics about stored shreds."""
        all_pipelines = set()
        for e in self._state.entries.values():
            all_pipelines.add(e.get("pipeline_id", ""))
        return {
            "total_shreds": len(self._state.entries),
            "unique_pipelines": len(all_pipelines),
        }

    def reset(self) -> None:
        """Clear all shreds and reset state."""
        self._state = PipelineDataShredderState()
