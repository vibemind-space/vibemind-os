...
@dataclass
class PipelineStepRetrierV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class PipelineStepRetrierV2:
    PREFIX = "psrtv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineStepRetrierV2State()
    ...
    def retry_v2(self, pipeline_id: str, step_name: str, max_attempts: int = 3, metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("retry_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id
    ...
    def get_retries(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_retries": len(entries), "unique_pipelines": len(unique)}
