...
@dataclass
class PipelineStepSequencerV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class PipelineStepSequencerV2:
    PREFIX = "pssv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineStepSequencerV2State()
    ...
    def sequence_v2(self, pipeline_id: str, step_name: str, order: int = 1, metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("sequence_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_sequences": len(entries), "unique_pipelines": len(unique)}
