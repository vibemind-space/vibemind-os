"""Pipeline step scaler — scales pipeline step parallelism and resources.

Manages scale records that control the number of replicas and resource
allocation for individual pipeline steps, enabling dynamic scaling
...
@dataclass
class PipelineStepScalerState:
    """Internal state for the PipelineStepScaler service."""
...
class PipelineStepScaler:
    """Scales pipeline step parallelism and resources.

    Creates and manages scale records that control how many replicas
    of a given pipeline step should run, along with optional metadata
    ...
    def __init__(self) -> None:
        self._state = PipelineStepScalerState()
    ...
    def scale(
        ...
        """Create a scale record for a pipeline step. Returns record ID."""
        self._prune()
    ...
    def get_scale(self, record_id: str) -> Optional[dict]:
        """Get a single scale record by ID. Returns None if not found."""
    ...
    def get_scales(
        ...
        """Get scale records, newest first. Optionally filter by pipeline_id."""
    ...
    def get_scale_count(self, pipeline_id: str = "") -> int:
        """Count scale records, optionally filtering by pipeline_id."""
        if not pipeline_id:
