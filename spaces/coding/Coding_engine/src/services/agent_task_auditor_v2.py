...
@dataclass
class AgentTaskAuditorV2State:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentTaskAuditorV2:
    PREFIX = "atav-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = AgentTaskAuditorV2State()
    ...
    def _fire(self, action: str, **detail):
    ...
    def audit_v2(
        ...
        self._prune()
        self._fire("audit_v2", task_id=task_id, record_id=record_id)
        return record_id

    def get_audit(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return copy.deepcopy(entry)

    def get_audits(self, agent_id: str = "", limit: int = 50) -> List[dict]:
        if agent_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_audits": len(self._state.entries),
