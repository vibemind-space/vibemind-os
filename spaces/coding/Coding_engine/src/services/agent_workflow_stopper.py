"""Agent Workflow Stopper -- stops workflows for agents.

Stores stop records for agent workflows.  Each stop record captures the
...
@dataclass
class AgentWorkflowStopperState:
    """Internal store for workflow stop entries."""
...
class AgentWorkflowStopper:
    """Stops workflows for agents.

    Each stop record tracks which agent stopped which workflow, along with
    ...
    def __init__(self) -> None:
        self._state = AgentWorkflowStopperState()
    ...
    def stop_workflow(
        ...
        self._prune()
        self._fire("stopped", entry)
        logger.debug(
            "Workflow stopped: %s agent=%s workflow=%s",
    ...
    def get_stop(self, record_id: str) -> Optional[dict]:
        """Get a stop record by its ID.  Returns dict or ``None``."""
    ...
    def get_stops(
        ...
        """Query stop records, newest first.
    ...
    def get_stop_count(self, agent_id: str = "") -> int:
        """Return the number of stop records, optionally filtered by *agent_id*."""
        if not agent_id:
    ...
    def get_stats(self) -> dict:
        """Return operational statistics for the stopper service."""
    ...
    def reset(self) -> None:
        """Clear all stored stop records, callbacks, and reset counters."""
        self._state = AgentWorkflowStopperState()
