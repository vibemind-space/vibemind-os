...
@dataclass
class PipelineStepBenchmarkerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineStepBenchmarker:
    PREFIX = "psbm-"
    MAX_ENTRIES = 10000

    def __init__(self):
        self._state = PipelineStepBenchmarkerState()
    ...
    def _fire(self, action: str, data: Any):
        if self._on_change is not None:
            try:
                ...
                logger.exception("Error in on_change callback for action=%s", action)
        for name, cb in list(self._state.callbacks.items()):
            try:
                cb(action, data)
            except Exception:
                logger.exception(
                    "Error in callback '%s' for action=%s", name, action
                )
    ...
    def benchmark(
        ...
        self._prune()
        self._fire("benchmarked", entry)
        return record_id

    def get_benchmark(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_benchmarks(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        if pipeline_id:
            entries = [
                e
    ...
    def get_benchmark_count(self, pipeline_id: str = "") -> int:
        if pipeline_id:
            return sum(
                1
                for e in self._state.entries.values()
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_benchmarks": len(self._state.entries),
