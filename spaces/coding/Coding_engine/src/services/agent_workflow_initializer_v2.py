"""Agent Workflow Initializer V2 -- initializes workflows for agents.
...
Uses SHA-256-based IDs with an ``awiv-`` prefix.
"""
...
@dataclass
class AgentWorkflowInitializerV2State:
...
class AgentWorkflowInitializerV2:
    """Initializes workflows for agents (V2).
    ...
    def __init__(self) -> None:
        self._state = AgentWorkflowInitializerV2State()
    ...
    def initialize_v2(
        ...
        """Initialize a workflow for an agent (v2).

        Returns the record ID (``awiv-`` prefix), or ``""`` if *agent_id*
        ...
        logger.debug(
            "Workflow initialized (v2): %s agent=%s workflow=%s",
