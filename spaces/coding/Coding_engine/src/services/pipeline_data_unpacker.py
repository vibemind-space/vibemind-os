"""Service module for unpacking pipeline data."""
...
@dataclass
class PipelineDataUnpackerState:
    entries: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataUnpacker:
    """Unpacks pipeline data."""

    PREFIX = "pdun-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataUnpackerState()
    ...
    def unpack(
        ...
        """Unpack pipeline data and return the record ID."""
        ...
        self._prune()
        self._fire("unpacked", record)
        return record_id

    def get_unpack(self, record_id: str) -> Optional[dict]:
        """Return the unpack record for *record_id*, or ``None``."""
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_unpacks(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        """Return unpacks, newest first, optionally filtered by *pipeline_id*."""
    ...
    def get_unpack_count(self, pipeline_id: str = "") -> int:
        """Return the number of unpacks, optionally filtered by *pipeline_id*."""
        if not pipeline_id:
            return len(self._state.entries)
        return sum(1 for e in self._state.entries.values() if e.get("pipeline_id") == pipeline_id)

    def get_stats(self) -> dict:
        """Return summary statistics about stored unpacks."""
        all_pipelines = set()
        for e in self._state.entries.values():
            all_pipelines.add(e.get("pipeline_id", ""))
        return {
            "total_unpacks": len(self._state.entries),
