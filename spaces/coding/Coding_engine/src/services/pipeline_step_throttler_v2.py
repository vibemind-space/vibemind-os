...
@dataclass
class PipelineStepThrottlerV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineStepThrottlerV2:
    PREFIX = "pstv2-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineStepThrottlerV2State()
    ...
    def throttle_v2(
        ...
        self._prune()
        self._fire("throttle_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id
