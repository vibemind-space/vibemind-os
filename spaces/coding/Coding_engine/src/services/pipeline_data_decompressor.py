"""Service module for recording decompression operations on pipeline data."""
...
@dataclass
class PipelineDataDecompressorState:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineDataDecompressor:
    """Records decompression operations on pipeline data."""

    PREFIX = "pddc-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineDataDecompressorState()
    ...
    def decompress(
        ...
        """Record a decompression operation on pipeline data. Returns record_id."""
        ...
        self._prune()
        self._fire("decompress", {"record_id": record_id, "pipeline_id": pipeline_id})
        return record_id

    def get_decompression(self, record_id: str) -> Optional[dict]:
        """Return a decompression record by id, or None if not found."""
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_decompressions(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
        """List decompression records, newest first. Optionally filter by pipeline_id."""
    ...
    def get_decompression_count(self, pipeline_id: str = "") -> int:
        """Return count of decompression records, optionally filtered by pipeline_id."""
        if not pipeline_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_decompressions": len(self._state.entries),
