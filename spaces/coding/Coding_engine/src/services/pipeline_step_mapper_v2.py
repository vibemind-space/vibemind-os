...
@dataclass
class PipelineStepMapperV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)

class PipelineStepMapperV2:
    PREFIX = "psmv-"
    MAX_ENTRIES = 10000

    def __init__(self) -> None:
        self._state = PipelineStepMapperV2State()
    ...
    def map_v2(self, pipeline_id: str, step_name: str, target: str = "default", metadata: Optional[dict] = None) -> str:
        ...
        self._prune()
        self._fire("map_v2", pipeline_id=pipeline_id, record_id=record_id)
        return record_id

    def get_mapping(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return dict(entry)

    def get_mappings(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_stats(self) -> dict:
        ...
        return {"total_mappings": len(entries), "unique_pipelines": len(unique)}
