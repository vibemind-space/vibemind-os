...
@dataclass
class PipelineStepFlattenerState:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class PipelineStepFlattener:
    PREFIX = "psfl-"
    MAX_ENTRIES = 10000

    def __init__(self, on_change: Optional[Callable] = None) -> None:
        self._state = PipelineStepFlattenerState()
        self._on_change = on_change
        logger.info("PipelineStepFlattener initialised")
    ...
    def flatten(
        ...
        if not pipeline_id or not step_name:
            logger.warning("flatten called with empty pipeline_id or step_name")
            return ""
        ...
        logger.info("Flattened step %s for pipeline %s -> %s", step_name, pipeline_id, record_id)
        self._prune()
        self._fire("flatten", record_id=record_id, pipeline_id=pipeline_id)
        return record_id

    def get_flatten(self, record_id: str) -> Optional[dict]:
        entry = self._state.entries.get(record_id)
        if entry is None:
            return None
        return copy.deepcopy(entry)

    def get_flattens(self, pipeline_id: str = "", limit: int = 50) -> List[dict]:
    ...
    def get_flatten_count(self, pipeline_id: str = "") -> int:
        if not pipeline_id:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            "total_flattens": len(self._state.entries),
    ...
    def reset(self) -> None:
        ...
        logger.info("PipelineStepFlattener reset")
