...
@dataclass
class AgentWorkflowMigratorV2State:
    entries: Dict[str, dict] = field(default_factory=dict)
    _seq: int = 0
    callbacks: Dict[str, Callable] = field(default_factory=dict)


class AgentWorkflowMigratorV2:
    PREFIX = "awmv-"
    MAX_ENTRIES = 10000

    def __init__(self, _on_change: Optional[Callable] = None) -> None:
        self._state = AgentWorkflowMigratorV2State()
        self._on_change = _on_change
        logger.debug("AgentWorkflowMigratorV2 initialised")
    ...
    def _prune(self) -> None:
        ...
        logger.info("Pruned %d migration entries", len(to_remove))
    ...
    def migrate_v2(
        ...
        if not agent_id or not workflow_name:
            logger.warning("migrate_v2 called with empty agent_id or workflow_name")
            return ""
        ...
        logger.info("Migrated workflow %s for agent %s -> %s", workflow_name, agent_id, record_id)
        self._prune()
        self._fire("migrate", record_id=record_id, agent_id=agent_id)
        return record_id
    ...
    def get_migrations(self, agent_id: str = "", limit: int = 50) -> List[dict]:
        results = [
            copy.deepcopy(e)
    ...
    def reset(self) -> None:
        ...
        logger.info("AgentWorkflowMigratorV2 reset")
