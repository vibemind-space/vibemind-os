"""Pipeline step enabler — enables/activates pipeline steps.

Tracks which pipeline steps have been enabled, along with the reason and
optional metadata, enabling audit trails and enablement analytics.
"""
...
@dataclass
class PipelineStepEnablerState:
    """Internal state for the PipelineStepEnabler service."""
...
class PipelineStepEnabler:
    """Enables/activates pipeline steps.

    Captures the pipeline ID, step name, reason, and optional metadata
    for every enabled step, supporting queries and statistics.
    """
    ...
    def __init__(self) -> None:
        self._state = PipelineStepEnablerState()
    ...
    def enable(
        ...
        """Enable a pipeline step. Returns enablement ID."""
        self._prune()
        enablement_id = self._generate_id()
        now = time.time()
        entry = {
        ...
        self._state.entries[enablement_id] = entry
        self._fire("enabled", dict(entry))
        return enablement_id

    # ------------------------------------------------------------------
    # get_enablement
    # ------------------------------------------------------------------

    def get_enablement(self, enablement_id: str) -> Optional[dict]:
        """Get a single enablement record by ID. Returns None if not found."""
        entry = self._state.entries.get(enablement_id)
        if entry is None:
            return None
        return dict(entry)
    ...
    def get_enablements(
        ...
        """Get enablement records, newest first. Optionally filter by pipeline_id."""
    ...
    def get_enablement_count(self, pipeline_id: str = "") -> int:
        """Count enablement records, optionally filtering by pipeline_id."""
        if not pipeline_id:
