...
class RoutingConfig:
    ...
    gate_temp: float = 1.0          # Softmax temperature (τ_g) - Higher = more distributed routing
...
class RoutedNeuroSymbolicBrain(nn.Module):
    ...
    def forward(
        ...
        # The brain outputs various features; we'll use policy_features from each module
        module_outputs = []
        for module_name in self.routing_config.module_names:
            # Get module features if available
            if f'{module_name.lower()}_features' in brain_output:
                module_outputs.append(brain_output[f'{module_name.lower()}_features'])
            else:
                # Fallback: use a shared representation
