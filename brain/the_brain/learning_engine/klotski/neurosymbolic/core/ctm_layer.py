...
class CTMLayer(nn.Module):
    ...
    def __init__(
        ...
        # Store base threshold for adaptive adjustment
        self._base_threshold = certainty_threshold

    def set_adaptive_threshold(self, complexity: float = 0.5, urgency: float = 0.5) -> float:
        """
        Adjust certainty threshold based on task characteristics.

        Higher complexity → lower threshold (think longer before stopping)
        Higher urgency → higher threshold (stop earlier, respond faster)
        ...
        # - Complex tasks: threshold drops to allow more thinking (min 0.5)
        # - Urgent tasks: threshold rises to stop earlier (max 0.95)
        complexity_factor = 1.0 - (complexity * 0.4)  # 1.0 to 0.6
        urgency_factor = 1.0 + (urgency * 0.2)        # 1.0 to 1.2

        # Calculate new threshold
        new_threshold = self._base_threshold * complexity_factor * urgency_factor
        new_threshold = max(0.5, min(0.95, new_threshold))  # Clamp to [0.5, 0.95]

        self.certainty_threshold = new_threshold
        return new_threshold

    def reset_threshold(self):
        """Reset to base threshold."""
        self.certainty_threshold = self._base_threshold
