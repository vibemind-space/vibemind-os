...
class PuzzleState:
    ...
    def get_state_hash(self) -> str:
        """
        Get canonical hash for this state.

        Uses board-based representation where same-shaped pieces are equivalent,
        except for the 2x2 goal piece (G) which is always marked.
        This produces the correct 25,955 unique states for classic Klotski.

        Returns:
            String hash representing the board configuration
        """
        # Create a 4x5 board grid
        board = [[0 for _ in range(self.board_width)] for _ in range(self.board_height)]

        # Mark cells with piece type:
        # G (2x2 goal) = 4
        # 1x2 vertical = 2
        # 2x1 horizontal = 3
        # 1x1 small = 1
        ...
        for piece in self.pieces.values():
            if piece.w == 2 and piece.h == 2:
                cell_type = 4  # Goal piece (unique)
            elif piece.w == 1 and piece.h == 2:
                cell_type = 2  # Vertical
            elif piece.w == 2 and piece.h == 1:
                cell_type = 3  # Horizontal
            else:
                cell_type = 1  # Small 1x1
